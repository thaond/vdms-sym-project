        ...
        public static List<IOStockVehicle> GetIOReportData(string DealerCode, string wCode, string itemCode, string itemType, string fromDt, string toDt)
        {
            ...
            var query = rptItems.GroupBy(iv => new { iv.ItemCode, iv.DealerCode, iv.BranchCode })
                                ...
                                (iv, i) => new IOStockVehicle
                        {
                            ...
                            BeginInv = dc.SaleInventories.SingleOrDefault(ivb => ivb.DealerCode == iv.DealerCode && ivb.BranchCode == iv.BranchCode //&& ivb.Quantity > 0
                                                     && ivb.ItemCode == iv.ItemCode && ivb.Month == dtStartOfBegin.Month && ivb.Year == dtStartOfBegin.Year),
                            ...
                            BeginActs = (int)dc.SaleInventoryDays
                                          ...
                                          .Sum(id => id.Quantity),
                            // order quantity
                            Order = dc.OrderDetails.Where(od => od.OrderHeader.DealerCode == DealerCode)
                                      ...
                                      .Where(od => od.ItemCode == i.ItemCode && od.OrderHeader.SecondaryShippingCode == iv.BranchCode)
                                      .Sum(od => od.OrderQty),
                            // import quantity
            ...
            var query = rptItems.GroupBy(iv => new { iv.ItemCode, iv.DealerCode })
                           ...
                           .Join(items, iv => iv.ItemCode, i => i.ItemCode,
                                (iv, i) => new IOStockVehicle
                                {
                                    ...
                                    BranchCode = "",
                                    Color = i.ColorName,
                                    BeginInvs = dc.SaleInventories.Where(ivb => ivb.DealerCode == iv.DealerCode //&& ivb.Quantity > 0
                                                             && ivb.ItemCode == iv.ItemCode && ivb.Month == dtStartOfBegin.Month && ivb.Year == dtStartOfBegin.Year).Sum(p => p.Quantity),
                                    // actions happened between the report day and the first day of report month
                                    BeginActs = (int)dc.SaleInventoryDays
                                                  .Where(id => id.DealerCode == DealerCode)
                                    ...
                                    Order = dc.OrderDetails.Where(od => od.OrderHeader.DealerCode == DealerCode)
                                              .Where(od => od.OrderHeader.OrderDate >= dtFrom && od.OrderHeader.OrderDate <= dtTo)
                                              .Where(od => od.OrderHeader.Status != (int)OrderStatus.Deleted && od.OrderHeader.Status != (int)OrderStatus.Draft)
                                              .Where(od => od.ItemCode == i.ItemCode)
                                              .Sum(od => od.OrderQty),
                                    // import quantity
                                    In = (int)dc.SaleInventoryDays.Where(id => id.DealerCode == DealerCode)
                                                .Where(id => id.ItemCode == i.ItemCode)
