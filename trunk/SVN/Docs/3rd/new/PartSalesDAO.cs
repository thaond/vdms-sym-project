...
namespace VDMS.II.PartManagement.Sales
{
	...
	public class PartSalesDAO : SessionPartDAO<PartSales>
	{
		...
		public static void LoadFromDB(long OrderId)
		{
			...
			foreach (var item in query)
			{
				list.Add(new PartSales
				{
                    ...
                    AvailableStock = item.i.CurrentStock -
                                                         (int)db.SalesDetails.Where(d => d.PartCode == item.d.PartCode &&
                                                                                    d.SalesHeader.Status == OrderStatus.OrderOpen &&
                                                                                    d.SalesHeader.DealerCode == UserHelper.DealerCode &&
                                                                                    d.SalesHeader.WarehouseId == UserHelper.WarehouseId).Sum(d => d.OrderQuantity),
		...
		{
			var db = DCFactory.GetDataContext<PartDataContext>();
			return (from sd in db.SalesDetails
					join ps in db.PartSafeties on sd.PartInfoId equals ps.PartInfoId
					where ps.WarehouseId == WarehouseId && sd.SalesHeaderId == OrderId && sd.OrderQuantity > (ps.CurrentStock - db.SalesDetails.Where(d => d.PartInfoId== ps.PartInfoId && 
                                                                                    d.SalesHeader.Status == OrderStatus.OrderOpen).Sum(d => d.OrderQuantity))
					select sd).Count() > 0;
		}
