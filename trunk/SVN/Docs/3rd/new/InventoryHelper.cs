...
namespace VDMS.I.Vehicle
{
    ...
    public class InventoryHelper
    {
        ...
        public static void DoInventory(string code, string dealerCode, int month, int year, VehicleDataContext dc)
        {
            ...
            if (udInvs.Count() > 0)
                dc.SaleInventories.InsertAllOnSubmit(udInvs);
            if (nwInvs.Count() > 0)
        ...
        public static void DoCloseW(string code, string dealerCode, int month, int year, VehicleDataContext dc)
        {
            ...
            // summarization
            InventoryHelper.DoInventory(code, dealerCode, (int)ilck.Month, (int)ilck.Year, dc);
        ...
        public static Inventoryday SaveInventoryDay(string ItemCode, DateTime ActionTime, int Quantity, int ActionType, string DealerCode, string BranchCode)
        {
            ...
            var item = daoItem.GetById(ItemCode, true);
        ...
        /// <summary>
        /// Get In/Out/Stock vehicle data
        /// </summary>
        /// <param name="DealerCode">Reporting dealer</param>
        /// <param name="wCode">Reporting warehouse code</param>
        /// <param name="itemCode">Item code to search</param>
        /// <param name="itemType">Item type to search</param>
        /// <param name="fromDt">Report from date</param>
        /// <param name="toDt">Report to date</param>
        /// <returns></returns>
        public static List<IOStockVehicle> GetIOReportData(string DealerCode, string wCode, string itemCode, string itemType, string fromDt, string toDt)
        {
            ...
            var tranHis = dc.SaleTransHistories.Where(id => id.ItemInstance.DealerCode == DealerCode &&
                                                            id.ItemInstance.BranchCode == wCode &&
                                                            ...
                                                            id.TransactionType == (int)ItemStatus.Return)).ToList();
            ...
            //if (!string.IsNullOrEmpty(wCode)) inv = inv.Where(iv => iv.BranchCode == wCode);

            // cong don tat ca dung cai nay
            //rptItems = inv.GroupBy(iv => new { iv.ItemCode, iv.DealerCode, iv.BranchCode })
            //               .Select(g => new IOReportItem() { ItemCode = g.Key.ItemCode, DealerCode = g.Key.DealerCode, BranchCode = g.Key.BranchCode }).ToList();
            // tinh tung` kho dung cai nay
            rptItems = inv.GroupBy(iv => new { iv.ItemCode, iv.DealerCode })
                           .Select(g => new IOReportItem() { ItemCode = g.Key.ItemCode, DealerCode = g.Key.DealerCode, BranchCode = wCode }).ToList();

            // get items(new items)
            var invd = tranHis.Where(i => i.TransactionDate >= dtFrom).Select(i => new { ItemCode = i.ItemInstance.ItemCode, DealerCode = i.ItemInstance.DealerCode, BranchCode = i.ItemInstance.BranchCode }).ToList();

            //if (!string.IsNullOrEmpty(wCode)) invd = invd.Where(iv => iv.BranchCode == wCode);
            rptItems.AddRange(invd.GroupBy(iv => new { iv.ItemCode, iv.DealerCode, iv.BranchCode })
                           .Select(g => new IOReportItem() { ItemCode = g.Key.ItemCode, DealerCode = g.Key.DealerCode, BranchCode = g.Key.BranchCode }).ToList());
            ...
            var aaa = rptItems.GroupBy(iv => new { iv.BranchCode });

            var query = rptItems.GroupBy(iv => new { iv.ItemCode, iv.DealerCode, iv.BranchCode })
                           .Select(g => new { g.Key.ItemCode, g.Key.DealerCode, g.Key.BranchCode })
                           .Join(items, iv => iv.ItemCode, i => i.ItemCode,
                                (iv, i) => new IOStockVehicle
                                {
                                    ...
                                    ItemType = i.ItemType,
                                    ItemCode = i.ItemCode,
                                    BranchCode = iv.BranchCode,
                                    Color = i.ColorName,
                                    //BeginMonth = dc.SaleInventories.SingleOrDefault(ivb => ivb.DealerCode == iv.DealerCode && ivb.BranchCode == iv.BranchCode
                                    //                         && ivb.ItemCode == iv.ItemCode && ivb.Month == dtStartOfBegin.Month && ivb.Year == dtStartOfBegin.Year).Quantity,
                                    // begin inventory data(at the first day of reporting month) of each item 
                                    // (may be null so cannot get quantity directly)
                                    BeginInv = inventories.SingleOrDefault(ivb => ivb.ItemCode == iv.ItemCode),
                                    ...
                                    //              .Where(id => id.ItemCode == i.ItemCode && id.BranchCode == iv.BranchCode)
                                    //              .Where(id => id.ActionDay >= startMonth && id.ActionDay < startDay)
                                    //              .Sum(id => id.Quantity),
                                    BeginActs = (int)tranHis.Where(id => id.ItemInstance.ItemCode == i.ItemCode && id.TransactionDate < dtFrom && IsImportedItem(id.TransactionType))
                                                                                                .Count()
                                                                                    - (int)tranHis.Where(id => id.ItemInstance.ItemCode == i.ItemCode && id.TransactionDate < dtFrom && !IsImportedItem(id.TransactionType))
                                                                                                  .Count(),
                                    // order quantity
                                    Order = orders.Where(od => od.ItemCode == i.ItemCode && od.OrderHeader.SecondaryShippingCode == iv.BranchCode)
                                                  .Sum(od => od.OrderQty),
                                    // import quantity
                                    ...
                                    In = (int)tranHis.Where(id => IsImportedItem(id.TransactionType) &&
                                                                      id.ItemInstance.ItemCode == i.ItemCode &&
                                                                      id.TransactionDate >= dtFrom)
                                                         .Count(),

                                    //In = (int)dc.SaleInvoices.Where(si => si.DealerCode == DealerCode &&
                                    //                                      si.
                                    ...
                                    //            .Where(id => id.ActionDay >= startDay && id.ActionDay <= endDay && id.Quantity < 0)
                                    //            .Sum(id => id.Quantity),
                                    Out = -(int)tranHis.Where(id => id.ItemInstance.ItemCode == i.ItemCode && id.TransactionDate >= dtFrom && !IsImportedItem(id.TransactionType))
                                                            .Count(),
                                })
                // remove zero items (all properties)
                        .Where(i => i.BeginInv != null || i.BeginActs != 0 || i.In != 0 || i.Order != 0 || i.Out != 0)
                        ;
        ...
        public static List<IOStockVehicle> GetIOReportData(string DealerCode, string itemCode, string itemType, string fromDt, string toDt)
        {
            ...
            var orders = dc.OrderDetails.Where(od => od.OrderHeader.DealerCode == DealerCode)
                                              .Where(od => od.OrderHeader.OrderDate >= dtFrom && od.OrderHeader.OrderDate <= dtTo)
                                              .Where(od => od.OrderHeader.Status != (int)OrderStatus.Deleted && od.OrderHeader.Status != (int)OrderStatus.Draft);
            var inventories = dc.SaleInventories.Where(ivb => ivb.DealerCode == DealerCode &&
                                                            ivb.Month == dtStartOfBegin.Month &&
                                                            ivb.Year == dtStartOfBegin.Year).ToList();
            ...
            var invd = tranHis.Where(i => i.TransactionDate >= dtFrom).Select(i => new { ItemCode = i.ItemInstance.ItemCode, DealerCode = i.ItemInstance.DealerCode }).ToList();
