...
namespace VDMS.II.Report
{
    ...
    public class InOutStockReport
    {
        ...
        public static IList<PartInOutStock> GetReportSource(string partType, DateTime dtFrom, DateTime dtTo, long? warehouseId, string dealerCode)
        {
            ...
            var transHis = dc.TransactionHistories.Where(th => th.TransactionDate >= DataFormat.DateOfFirstDayInMonth(dtFrom) && th.TransactionDate <= dtTo).ToList();
            var invs = dc.Inventories.Where(bi => bi.Year == dtBegin.Year && bi.Month == dtBegin.Month).ToList();
            ...
            if (warehouseId != null)
            {
                ...
                var query = dc.PartInfos.Where(pi => pi.PartType == partType && pi.DealerCode == dealerCode)
                                        .Where(pi => pi.PartSafeties.Where(ps => ps.WarehouseId == warehouseId).Count() > 0).ToList()
                                        // left join equivalent
                                        .GroupJoin(dc.Accessories,
                                                   p => p.AccessoryId,
                                                   a => a.AccessoryId,
                                                   (p, a) => new
                                                   {
                                                       p.PartInfoId,
                                                       p.PartCode,
                                                       PartName = a.SingleOrDefault() == null ? "" : a.SingleOrDefault().VietnamName,
                                                   })
                                        .GroupJoin(invs,
                                                   p => p.PartInfoId,
                                                   i => i.PartInfoId,
                                                   (p, i) => new
                                                   {
                                                       ...
                                                       p.PartCode,
                                                       p.PartName,
                                                       BeginMonth = i.SingleOrDefault(bi => bi.WarehouseId == (long)warehouseId) == null ? 0 : i.SingleOrDefault(bi => bi.WarehouseId == (long)warehouseId).Quantity,
                                                   })
                                        ...
                                        .GroupJoin(transHis,
                                                  ...
                                                  (p, t) => new
                                                  {
                                                      ...
                                                      p.PartName,
                                                      p.BeginMonth,
                                                      BeginDay = t.Where(th => th.TransactionDate < dtFrom)
                                                                  .Where(th => th.WarehouseId == (long)warehouseId)
                                                                  .Sum(th => th.Quantity),
                                                      In = t.Where(th => th.TransactionDate >= dtFrom &&
                                                                         th.WarehouseId == (long)warehouseId &&
                                                                         th.Quantity > 0).Sum(th => th.Quantity),
                                                      Out = t.Where(th => th.TransactionDate >= dtFrom)
                                                             .Where(th => th.WarehouseId == (long)warehouseId)
                                                             .Where(th => th.Quantity < 0)
                                                             .Sum(th => th.Quantity) * -1,
                                                      InAmout = t.Where(th => th.TransactionDate >= dtFrom)
                                                                 .Where(th => th.WarehouseId == (long)warehouseId)
                                                                 .Where(th => th.Quantity > 0)
                                                                 .Sum(th => th.ActualCost),
                                                      OutAmount = t.Where(th => th.TransactionDate >= dtFrom)
                                                                   .Where(th => th.WarehouseId == (long)warehouseId)
                                                                   .Where(th => th.Quantity < 0)
                                                                   .Sum(th => th.ActualCost),
                                                  }).ToList();
                ...
                if (partType == PartType.Part)
                {
                    list = query.Join(dc.Parts.Where(p => p.DatabaseCode == d.DatabaseCode), pi => pi.PartCode, p => p.PartCode,
                        (pi, p) => new PartInOutStock()
                        {
                            ...
                            PartCode = p.PartCode,
                            PartName = p.VietnamName,
                            BeginMonth = pi.BeginMonth,
                            BeginDay = pi.BeginDay,
                            In = pi.In,
                            Out = pi.Out,
                            InAmount = pi.InAmout,
                            OutAmount = pi.OutAmount,
                        })
            ...
            else
            {
                ...
                var query = dc.PartInfos.Where(pi => pi.PartType == partType).ToList()
                                        .GroupJoin(dc.Accessories,
                                                   p => p.AccessoryId,
                                                   a => a.AccessoryId,
                                                   (p, a) => new
                                        ...
                                        .GroupJoin(invs,
                                                   ...
                                                   (p, i) => new
                                                   {
                                                       ...
                                                       p.PartCode,
                                                       p.PartName,
                                                       BeginMonth = i.SingleOrDefault(bi => bi.DealerCode == dealerCode && bi.WarehouseId == null) == null ? 0 : i.SingleOrDefault(bi => bi.DealerCode == dealerCode && bi.WarehouseId == null).Quantity,
                                                   })
