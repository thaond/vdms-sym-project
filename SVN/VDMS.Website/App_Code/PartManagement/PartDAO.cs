...
namespace VDMS.II.PartManagement
{
    public partial class PartDAO
    {
                ...
                case InventoryAction.StockTransfer:
                    if (partType == PartType.Part)
                    {
                        var query = from t in db.Parts
                                    ...
                                    join t1 in db.PartSafeties on t0.PartInfoId equals t1.PartInfoId
                                    //join t2 in db.SalesDetails.Where(d => d.SalesHeader.Status == OrderStatus.OrderOpen &&
                                    //                                      d.SalesHeader.DealerCode == UserHelper.DealerCode &&
                                    //                                      d.SalesHeader.WarehouseId == whId).DefaultIfEmpty()
                                    //        on t.PartCode equals t2.PartCode into details
                                    where t0.PartType == partType && t1.WarehouseId == whId && t1.CurrentStock > 0 && t.DatabaseCode == UserHelper.DatabaseCode
                                    let curr = t1.CurrentStock
                                    select new
                                    {
                                        t0.PartInfoId,
                                        t0.PartCode,
                                        ...
                                        ReservedStock = db.SalesDetails.Where(d => d.SalesHeader.Status == OrderStatus.OrderOpen &&
                                                                                                      d.SalesHeader.DealerCode == UserHelper.DealerCode &&
                                                                                                      d.SalesHeader.WarehouseId == whId &&
                                                                                                      d.PartCode == t.PartCode).Sum(d => d.OrderQuantity),
                                        Model = string.Empty
                                    };
                        ...
                        allPart = query.Count();
                        return query.OrderBy(p => p.PartCode)
                                    .Skip(startRowIndex)
                                    .Take(maximumRows)
                                    .ToList()
                                    .Select(q => new
                                                  {
                                                      ...
                                                      q.SafetyQuantity,
                                                      q.VietnamName,
                                                      AvailableStock = q.CurrentStock - q.ReservedStock,
                                                  });
                    }
                    else
                    {
                        var query = from t in db.Accessories
                                    ...
                                    select new
                                    {
                                        ...
                                        ReservedStock = db.SalesDetails.Where(d => d.SalesHeader.Status == OrderStatus.OrderOpen &&
                                                                                                      d.SalesHeader.DealerCode == UserHelper.DealerCode &&
                                                                                                      d.SalesHeader.WarehouseId == whId &&
                                                                                                      d.PartInfoId == t.AccessoryId).Sum(d => d.OrderQuantity),
                                        Model = string.Empty
                                    };
                        allPart = query.Count();
                        return query.OrderBy(p => p.PartCode)
        ...
        public static bool IsPartCodeValid(string PartCode, string PartType, string DealerCode)
        {
            var db = DCFactory.GetDataContext<PartDataContext>();
            if (PartType == "P")
                return VDMS.Data.TipTop.Part.IsPartExist(PartCode, UserHelper.DatabaseCode, false);
        ...
        public static bool IsPartCodeValid(string PartCode, string PartType, bool checkStatus)
        {
            var db = DCFactory.GetDataContext<PartDataContext>();
            if (PartType == "P")
                return VDMS.Data.TipTop.Part.IsPartExist(PartCode, UserHelper.DatabaseCode, checkStatus);
        ...
        public static bool IsPartCodeValid(string PartCode, bool checkStatus)
        {
            return IsPartCodeValid(PartCode, "P", checkStatus);
        }
        ...
        public static bool IsPartCodeValidForCC(string PartCode, string partType)
        {
            if (partType == PartType.Part)
            {
                ...
                return VDMS.Data.TipTop.Part.IsPartExist(PartCode, "all", false); // dqminh Jul 10
            }
            else
            {
                return IsPartCodeValid(PartCode, partType, false);
            }
        }
