...
namespace VDMS.II.Report
{
    ...
    public class PartMonthlyReport : IDisposable
    {
        ...
        public List<MonthlyReportPart> GetMonthlyReportItems(Warehouse wh, Dealer dealer, int month, int year)
        {
            ...
            var warehouses = dc.ActiveWarehouses.Where(w => w.DealerCode == dealer.DealerCode && w.Type == WarehouseType.Part)
                                    .GroupJoin(dc.PartSafeties,
                                               w => w.WarehouseId,
                                               s => s.WarehouseId,
                                               (w, s) => new
                                               {
                                                   ...
                                                   w.Code,
                                                   w.Address,
                                                   PartSafetyCount = s.Count(),
                                               })
                                    .Where(w => w.PartSafetyCount > 0).ToList();
            var transactions = dc.TransactionHistories.Where(th => th.TransactionDate >= dtTHBegin && th.TransactionDate < dtTHEnd && warehouses.Select(w => w.WarehouseId).Contains(th.WarehouseId))
                                                      .GroupJoin(dc.PartInfos,
                                                             th => th.PartInfoId,
                                                             p => p.PartInfoId,
                                                             (th, p) => new
                                                             {
                                                                 ...
                                                                 th.WarehouseId,
                                                                 th.Quantity,
                                                                 p.SingleOrDefault().PartCode,
                                                                 p.SingleOrDefault().PartType,
                                                             }).ToList();
            var inventories = dc.Inventories.Where(iv => (iv.Month == month || iv.Month == bgMonth) && iv.Year == bgYear && warehouses.Select(w => w.WarehouseId).Contains(iv.WarehouseId.GetValueOrDefault(0)))
                                            .GroupJoin(dc.PartInfos,
                                                  iv => iv.PartInfoId,
                                                  p => p.PartInfoId,
                                                  (iv, p) => new
            ...
            if (wh == null)
            {
                ...
                res = pisP.Join(dc.Parts.Where(p => p.DatabaseCode == dbCode), pi => pi.PartCode, p => p.PartCode,
                    (pi, p) => new MonthlyReportPart
                    {
                        ...
                        // warehouses
                        ByWarehouses = warehouses.Count() > 0 ? warehouses.Select(pl => new MonthlyReportItem
                        {
                            PlaceCode = pl.Code,
                            PlaceName = pl.Address,
                            PlaceType = "W",
                            BeginQuantity = dc.Inventories.SingleOrDefault(iv => iv.DealerCode == dealer.DealerCode &&
                                                                                 iv.PartInfo.PartType == PartType.Part &&
                                                                                 iv.PartInfo.PartCode == pi.PartCode &&
                                                                                 iv.Month == bgMonth &&
                                                                                 iv.Year == year &&
                                                                                 iv.WarehouseId == pl.WarehouseId) == null ? 0 : dc.Inventories.SingleOrDefault(iv => iv.DealerCode == dealer.DealerCode &&
                                                                                                                                                                         iv.PartInfo.PartType == PartType.Part &&
                                                                                                                                                                         iv.PartInfo.PartCode == pi.PartCode &&
                                                                                                                                                                         iv.Month == bgMonth &&
                                                                                                                                                                         iv.Year == year &&
                                                                                                                                                                         iv.WarehouseId == pl.WarehouseId ).Quantity,
                            Balance = 0,//inventories.FirstOrDefault(iv => iv.Month == month &&  iv.PartType == PartType.Part && iv.PartCode == pi.PartCode && iv.WarehouseId == pl.WarehouseId).Quantity,
                            InQuantity = 0,//transactions.Where(th => th.PartType == PartType.Part && th.PartCode == pi.PartCode && th.Quantity > 0 && th.WarehouseId == pl.WarehouseId).Count(),
                            InAmount = 0,//transactions.Where(th => th.PartType == PartType.Part && th.PartCode == pi.PartCode && th.Quantity > 0 && th.WarehouseId == pl.WarehouseId).Count(),
                            OutQuantity = 0,//transactions.Where(th => th.PartType == PartType.Part && th.PartCode == pi.PartCode && th.Quantity < 0 && th.WarehouseId == pl.WarehouseId).Count(),
                            OutAmount = 0,//transactions.Where(th => th.PartType == PartType.Part && th.PartCode == pi.PartCode && th.Quantity < 0 && th.WarehouseId == pl.WarehouseId).Count(),
                        }) : null,
                        // sub dealers
                ...
                res.AddRange(pisA.Select(pi => new MonthlyReportPart
                {
                    ...
                    ByWarehouses = warehouses.Count() > 0 ? warehouses.Select(pl => new MonthlyReportItem
                    {
                        ...
                        PlaceName = pl.Address,
                        PlaceType = "W",
                        BeginQuantity = 0,//inventories.FirstOrDefault(iv => iv.Month == bgMonth && iv.PartType == PartType.Accessory && iv.PartCode == pi.PartCode && iv.WarehouseId == pl.WarehouseId).Quantity,
                        Balance = 0,//inventories.FirstOrDefault(iv => iv.Month == month && iv.PartType == PartType.Accessory && iv.PartCode == pi.PartCode && iv.WarehouseId == pl.WarehouseId).Quantity,
                        InQuantity = 0,//transactions.Where(th => th.PartType == PartType.Accessory && th.PartCode == pi.PartCode && th.Quantity > 0 && th.WarehouseId == pl.WarehouseId).Sum(th => th.Quantity),
                        InAmount = 0,//transactions.Where(th => th.PartType == PartType.Accessory && th.PartCode == pi.PartCode && th.Quantity > 0 && th.WarehouseId == pl.WarehouseId).Sum(th => th.ActualCost),
                        OutQuantity = 0,//transactions.Where(th => th.PartType == PartType.Accessory && th.PartCode == pi.PartCode && th.Quantity < 0 && th.WarehouseId == pl.WarehouseId).Sum(th => th.Quantity),
                        OutAmount = 0,//transactions.Where(th => th.PartType == PartType.Accessory && th.PartCode == pi.PartCode && th.Quantity < 0 && th.WarehouseId == pl.WarehouseId).Sum(th => th.ActualCost),
                    }) : null,
                    ByDealers = dc.Dealers.Where(d => d.ParentCode == dealer.DealerCode
                                            && d.PartInfos.Where(dp => dp.PartCode == pi.PartCode).Count() > 0
                                            ).Select(pl => new MonthlyReportItem
