...
public partial class Part_Sale : BasePage
{
	...
	bool CheckPartNo(bool CheckAll)
	{
		...
		if (!CheckAll)
		{
			foreach (GridViewRow row in gv1.Rows)
			{
				...
				if (!string.IsNullOrEmpty(s) && (!PartDAO.IsPartCodeValid(s, t) || string.IsNullOrEmpty(name)))
				{
					row.CssClass = "error";
					r = false;
				}
		...
		else
		{
			...
			foreach (var item in PartSalesDAO.Parts)
			{
				if (!string.IsNullOrEmpty(item.PartCode) && (!PartDAO.IsPartCodeValid(item.PartCode, item.PartType) || string.IsNullOrEmpty(item.PartName)))
				{
	...
	protected void gv1_DataBound(object sender, EventArgs e)
	{
		...
		if (gv.FooterRow != null)
		{
			...
			{
				gv.FooterRow.Cells[4].Text = string.Format("{0}", PartSalesDAO.Parts.Sum(p => p.Quantity));
				gv.FooterRow.Cells[4].CssClass = "number";
				gv.FooterRow.Cells[7].Text = string.Format("{0:C0}", PartSalesDAO.Parts.Sum(p => p.Amount));
				gv.FooterRow.Cells[7].CssClass = "number";
			}
		}
	}
